...
export function buildTargetDiscoveryPrompt(input: {
  ...
  const chunks: string[] = [
    ...
    "",
    "brrrsentry always generates a runnable Go harness automatically.",
    "Prefer targets that are easy to auto-wire (fast path):",
    ...
    "",
    "If there are not enough fast-path targets, also return complex but fuzzable Go targets (slow path):",
    "- exported methods or functions with multiple args",
    "- targets where a receiver can be constructed from the fuzz bytes",
    "- targets that accept structs that can be derived from bytes",
    "",
